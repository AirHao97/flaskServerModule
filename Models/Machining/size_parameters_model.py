...
class SizeParameters(db.Model):

    __tablename__ = 'machining_size_parameters'

    id = db.Column(db.Text, primary_key=True, doc='尺寸参数ID')
    name = db.Column(db.Text, unique=True, doc='尺寸参数名称')
    label = db.Column(db.Text, unique=True, nullable=True, doc='尺寸参数通用标识')
    mark = db.Column(db.Text, doc='备注')
    pic_address = db.Column(db.Text, nullable=True, doc='图片地址')

    # 关联创建者
    creator = db.relationship('User', backref='sizeParameters')
