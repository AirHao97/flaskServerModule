...
class Material(db.Model):
    ...
    __tablename__ = 'machining_material'

    id = db.Column(db.Text, primary_key=True, doc='材料ID')
    name = db.Column(db.Text, unique=True, doc='材料名称')
    label = db.Column(db.Text, unique=True, nullable=True, doc='材料行业通用标识')
    mark = db.Column(db.Text, doc='备注')
    pic_address = db.Column(db.Text, nullable=True, doc='图片地址')

    # 关联创建者
    creator = db.relationship('User', backref='Materials')
