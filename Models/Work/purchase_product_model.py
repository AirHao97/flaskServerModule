...
class PurchaseProduct(db.Model):
    ...
    sku = db.Column(db.Text, unique=True,  index = True, doc='sku')

    # 入库人、出库人
    stock_in_id = db.Column(db.Text, db.ForeignKey('user.id'))
    stock_in_person = db.relationship('User', backref='stock_in_purchase_products',foreign_keys=[stock_in_id])
    stock_out_id = db.Column(db.Text, db.ForeignKey('user.id'))
    stock_out_person = db.relationship('User', backref='stock_out_purchase_products',foreign_keys=[stock_out_id])
