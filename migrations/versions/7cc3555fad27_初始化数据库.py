"""初始化数据库

Revision ID: 7cc3555fad27
Revises: 
Create Date: 2024-12-24 15:42:35.678147

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '7cc3555fad27'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('department',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('name', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('name')
    )
    op.create_table('team',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('name', sa.Text(), nullable=True),
    sa.Column('department_id', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['department_id'], ['department.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('name')
    )
    op.create_table('user',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('username', sa.Text(), nullable=False),
    sa.Column('password', sa.Text(), nullable=False),
    sa.Column('email', sa.Text(), nullable=True),
    sa.Column('telephone_number', sa.Text(), nullable=True),
    sa.Column('is_admin', sa.Boolean(), nullable=True),
    sa.Column('is_department_admin', sa.Boolean(), nullable=True),
    sa.Column('is_team_admin', sa.Boolean(), nullable=True),
    sa.Column('is_active', sa.Boolean(), nullable=True),
    sa.Column('department_id', sa.Text(), nullable=True),
    sa.Column('team_id', sa.Text(), nullable=True),
    sa.Column('last_login_time', sa.DateTime(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['department_id'], ['department.id'], ),
    sa.ForeignKeyConstraint(['team_id'], ['team.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('user', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_user_username'), ['username'], unique=True)

    op.create_table('operate_log',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('operate_type', sa.Text(), nullable=True),
    sa.Column('describe', sa.Text(), nullable=True),
    sa.Column('operator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['operator_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('package',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('sku', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('sku')
    )
    op.create_table('partners_orders_association',
    sa.Column('user_id', sa.Text(), nullable=False),
    sa.Column('partner_id', sa.Text(), nullable=False),
    sa.ForeignKeyConstraint(['partner_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('user_id', 'partner_id')
    )
    op.create_table('partners_system_products_association',
    sa.Column('user_id', sa.Text(), nullable=False),
    sa.Column('partner_id', sa.Text(), nullable=False),
    sa.ForeignKeyConstraint(['partner_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('user_id', 'partner_id')
    )
    op.create_table('role',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('name', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('role', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_role_name'), ['name'], unique=True)

    op.create_table('shop',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('name', sa.Text(), nullable=True),
    sa.Column('api_id', sa.Text(), nullable=False),
    sa.Column('api_key', sa.Text(), nullable=False),
    sa.Column('owner_id', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['owner_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('api_id'),
    sa.UniqueConstraint('api_key'),
    sa.UniqueConstraint('name')
    )
    op.create_table('system_product',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('primary_image', sa.Text(), nullable=True),
    sa.Column('system_sku', sa.Text(), nullable=True),
    sa.Column('reference_weight', sa.Text(), nullable=True),
    sa.Column('reference_cost', sa.Text(), nullable=True),
    sa.Column('purchase_link', sa.Text(), nullable=True),
    sa.Column('purchase_platform', sa.Text(), nullable=True),
    sa.Column('price', sa.Text(), nullable=True),
    sa.Column('supplier_name', sa.Text(), nullable=True),
    sa.Column('productId_1688', sa.Text(), nullable=True),
    sa.Column('specId_1688', sa.Text(), nullable=True),
    sa.Column('skuId_1688', sa.Text(), nullable=True),
    sa.Column('father_id', sa.Text(), nullable=True),
    sa.Column('department_id', sa.Text(), nullable=True),
    sa.Column('creator_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['creator_id'], ['user.id'], ),
    sa.ForeignKeyConstraint(['department_id'], ['department.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('system_sku')
    )
    op.create_table('ozon_order',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('order_id', sa.Text(), nullable=True),
    sa.Column('order_number', sa.Text(), nullable=True),
    sa.Column('posting_number', sa.Text(), nullable=True),
    sa.Column('posting_status', sa.Text(), nullable=True),
    sa.Column('logistics_status', sa.Text(), nullable=True),
    sa.Column('delivery_id', sa.Text(), nullable=True),
    sa.Column('delivery_name', sa.Text(), nullable=True),
    sa.Column('delivery_tpl_provider_type', sa.Text(), nullable=True),
    sa.Column('delivery_tpl_provider_id', sa.Text(), nullable=True),
    sa.Column('delivery_tpl_provider_name', sa.Text(), nullable=True),
    sa.Column('warehouse_id', sa.Text(), nullable=True),
    sa.Column('warehouse_name', sa.Text(), nullable=True),
    sa.Column('tracking_number', sa.Text(), nullable=True),
    sa.Column('customer_id', sa.Text(), nullable=True),
    sa.Column('customer_name', sa.Text(), nullable=True),
    sa.Column('address_city', sa.Text(), nullable=True),
    sa.Column('in_process_at', sa.Text(), nullable=True),
    sa.Column('shipment_date', sa.Text(), nullable=True),
    sa.Column('delivering_date', sa.Text(), nullable=True),
    sa.Column('cancel_reason', sa.Text(), nullable=True),
    sa.Column('cancellation_type', sa.Text(), nullable=True),
    sa.Column('currency_code', sa.Text(), nullable=True),
    sa.Column('audit_in_system', sa.Boolean(), nullable=True),
    sa.Column('is_over_weight', sa.Boolean(), nullable=True),
    sa.Column('weight', sa.Text(), nullable=True),
    sa.Column('over_weight_record', sa.Text(), nullable=True),
    sa.Column('total_price', sa.Text(), nullable=True),
    sa.Column('system_status', sa.Text(), nullable=True),
    sa.Column('approval_time', sa.DateTime(), nullable=True),
    sa.Column('dispatch_time', sa.DateTime(), nullable=True),
    sa.Column('shop_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['shop_id'], ['shop.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('posting_number')
    )
    op.create_table('ozon_product',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('offer_id', sa.Text(), nullable=True),
    sa.Column('name', sa.Text(), nullable=True),
    sa.Column('price', sa.Text(), nullable=True),
    sa.Column('currency_code', sa.Text(), nullable=True),
    sa.Column('sku', sa.Text(), nullable=True),
    sa.Column('link', sa.Text(), nullable=True),
    sa.Column('mandatory_mark', sa.Text(), nullable=True),
    sa.Column('primary_image', sa.Text(), nullable=True),
    sa.Column('product_id', sa.Text(), nullable=True),
    sa.Column('fbo_commission_percent', sa.Text(), nullable=True),
    sa.Column('fbo_commission_value', sa.Text(), nullable=True),
    sa.Column('fbs_commission_percent', sa.Text(), nullable=True),
    sa.Column('fbs_commission_value', sa.Text(), nullable=True),
    sa.Column('rfbs_commission_percent', sa.Text(), nullable=True),
    sa.Column('rfbs_commission_value', sa.Text(), nullable=True),
    sa.Column('fbp_commission_percent', sa.Text(), nullable=True),
    sa.Column('fbp_commission_value', sa.Text(), nullable=True),
    sa.Column('category_two_id', sa.Text(), nullable=True),
    sa.Column('category_three_id', sa.Text(), nullable=True),
    sa.Column('shop_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['shop_id'], ['shop.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('ozon_product', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_ozon_product_sku'), ['sku'], unique=False)

    op.create_table('purchase_order',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('purchase_id', sa.Text(), nullable=True),
    sa.Column('fill_purchase_id_time', sa.DateTime(), nullable=True),
    sa.Column('price', sa.Text(), nullable=True),
    sa.Column('shipping_fee', sa.Text(), nullable=True),
    sa.Column('back_fee', sa.Text(), nullable=True),
    sa.Column('posting_numbers', sa.Text(), nullable=True),
    sa.Column('logistics_status', sa.Text(), nullable=True),
    sa.Column('packer_msg', sa.Text(), nullable=True),
    sa.Column('purchase_platform', sa.Text(), nullable=True),
    sa.Column('status', sa.Text(), nullable=True),
    sa.Column('platform_status', sa.Text(), nullable=True),
    sa.Column('is_error', sa.Boolean(), nullable=True),
    sa.Column('error_words', sa.Text(), nullable=True),
    sa.Column('now_turn', sa.Text(), nullable=True),
    sa.Column('system_product_id', sa.Text(), nullable=True),
    sa.Column('department_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['department_id'], ['department.id'], ),
    sa.ForeignKeyConstraint(['system_product_id'], ['system_product.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('user_roles',
    sa.Column('user_id', sa.Text(), nullable=True),
    sa.Column('role_id', sa.Text(), nullable=True),
    sa.ForeignKeyConstraint(['role_id'], ['role.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], )
    )
    op.create_table('ozon_order_ozon_product',
    sa.Column('order_id', sa.Text(), nullable=False),
    sa.Column('product_id', sa.Text(), nullable=False),
    sa.Column('price', sa.Text(), nullable=False),
    sa.Column('quantity', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['order_id'], ['ozon_order.id'], ),
    sa.ForeignKeyConstraint(['product_id'], ['ozon_product.id'], ),
    sa.PrimaryKeyConstraint('order_id', 'product_id')
    )
    op.create_table('ozon_product_system_product',
    sa.Column('ozon_product_id', sa.Text(), nullable=False),
    sa.Column('system_product_id', sa.Text(), nullable=False),
    sa.Column('quantity', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['ozon_product_id'], ['ozon_product.id'], ),
    sa.ForeignKeyConstraint(['system_product_id'], ['system_product.id'], ),
    sa.PrimaryKeyConstraint('ozon_product_id', 'system_product_id')
    )
    op.create_table('purchase_product',
    sa.Column('id', sa.Text(), nullable=False),
    sa.Column('price', sa.Text(), nullable=True),
    sa.Column('stock_in_date', sa.DateTime(), nullable=True),
    sa.Column('stock_out_date', sa.DateTime(), nullable=True),
    sa.Column('loss_date', sa.DateTime(), nullable=True),
    sa.Column('sku', sa.Text(), nullable=True),
    sa.Column('type', sa.Text(), nullable=True),
    sa.Column('status', sa.Text(), nullable=True),
    sa.Column('mark', sa.Text(), nullable=True),
    sa.Column('purchase_order_id', sa.Text(), nullable=True),
    sa.Column('ozon_order_id', sa.Text(), nullable=True),
    sa.Column('system_product_id', sa.Text(), nullable=True),
    sa.Column('package_id', sa.Text(), nullable=True),
    sa.Column('create_time', sa.DateTime(), nullable=True),
    sa.Column('modify_time', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['ozon_order_id'], ['ozon_order.id'], ),
    sa.ForeignKeyConstraint(['package_id'], ['package.id'], ),
    sa.ForeignKeyConstraint(['purchase_order_id'], ['purchase_order.id'], ),
    sa.ForeignKeyConstraint(['system_product_id'], ['system_product.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('purchase_product', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_purchase_product_sku'), ['sku'], unique=True)

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('purchase_product', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_purchase_product_sku'))

    op.drop_table('purchase_product')
    op.drop_table('ozon_product_system_product')
    op.drop_table('ozon_order_ozon_product')
    op.drop_table('user_roles')
    op.drop_table('purchase_order')
    with op.batch_alter_table('ozon_product', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_ozon_product_sku'))

    op.drop_table('ozon_product')
    op.drop_table('ozon_order')
    op.drop_table('system_product')
    op.drop_table('shop')
    with op.batch_alter_table('role', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_role_name'))

    op.drop_table('role')
    op.drop_table('partners_system_products_association')
    op.drop_table('partners_orders_association')
    op.drop_table('package')
    op.drop_table('operate_log')
    with op.batch_alter_table('user', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_user_username'))

    op.drop_table('user')
    op.drop_table('team')
    op.drop_table('department')
    # ### end Alembic commands ###
