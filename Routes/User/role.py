...
user_role_list = Blueprint('user_role', __name__, url_prefix='/user/role')

# 无登陆都可以查到总数据  # 登陆后 都可以看到详情页
# 只有登陆后有某个领域权限的专家 才能新增该领域菜单下的工艺卡片（其他库只有管理员能新增）

# 查询数据
@user_role_list.route('/getData', methods=['GET'])
@jwt_required()
def getData():
...
# 新增数据
@user_role_list.route('/addData', methods=['POST'])
@jwt_required()
def addData():
    return addDataFromDataBase(Role) 


# 修改数据
@user_role_list.route('/modifyData', methods=['POST'])
@jwt_required()
def modifyData(): 
    return modifyDataFromDataBase(Role)


# 删除数据
@user_role_list.route('/deleteData', methods=['POST'])
@jwt_required()
def deleteData():
    return deleteDataFromDataBase(Role)
